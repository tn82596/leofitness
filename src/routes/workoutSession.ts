...
// CREATE
router.post('/workout_session/:workout_session_id', (req: Request, res: Response, next: NextFunction) => {
    // TODO: create an insert a new workout session
	res.status(200).send({status: "success", data: dummy_data});
});

// UPDATE
router.put('/workout_session/:workout_session_id', (req: Request, res: Response, next: NextFunction) => {
	res.status(200).send({ status: 'success', data: dummy_data });
});

// DELETE
router.delete('/workout_session/:workout_session_id', (req: Request, res: Response, next: NextFunction) => {
	res.status(200).send({ status: 'success', data: dummy_data });
});
